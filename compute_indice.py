...
def compute_ACI(spectro,j_bin):
    ...
    times = range(0, int(spectro.shape[1]-10), int(j_bin)) # alternative time indices to follow the R code

    jspecs = [np.array(spectro[:,int(i):int(i+j_bin)]) for i in times]  # sub-spectros of temporal size j
